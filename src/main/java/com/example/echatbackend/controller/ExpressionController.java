...
public class ExpressionController extends BaseController {
    ...
    public ResponseEntity<Object> getExpression() {
        ...
        for (File expressionGroup : Objects.requireNonNull(expressions.listFiles())) {//表情按照类型分组为多个子文件夹
            if (expressionGroup.isDirectory()) {
                for (File expression : Objects.requireNonNull(expressionGroup.listFiles())) {
                    num++;
